...
public class Catalina {
    ...
    public void load() {
        ...
        //解析器对象  用于解析 server.xml文件的 （Digester起初是解析struts2的配置文件的 后来被放在appache的common包下）
        ...
        try {
            ...
            //如果server.xml  解析server-embed.xml 文件
        ...
        /*
         * 创建server后， 为server设置属相
         */
        getServer().setCatalina(this);
        getServer().setCatalinaHome(Bootstrap.getCatalinaHomeFile());//安装目录
        getServer().setCatalinaBase(Bootstrap.getCatalinaBaseFile());//工作目录
        ...
        // 启动server服务
        try {
            //  调用接口LifeCycle的init()方法进行初始化时 ，
            //  其实是 调用LifeCycle子类StandardServer的initInternal方法
            getServer().init();
        } catch (LifecycleException e) {
    ...
    /*
     *  当BootStrp .load() 方法被加载时 ， 就会通过反射 调用catalina的load()方法
     */
    ...
    public void start() {
        ...
        // Register shutdown hook ：安全的关闭服务器
        if (useShutdownHook) {
