...
public final class Bootstrap {
    ...
    /*
     *为什么使用类加载器 ？  ：将 .class加载到jvm内存
     *	applicationClassLoader 到jvm内存中加载.class
     *	extensionClassLoader   到JAVA_HOME\lib\ext 加载.class
     *  bootStrapClassLoader   到JAVA_HOME\lib\ 加载.class
     *
     *为什么tomcat要自定类加载器而不去使用jdk的类加载 ？  ：由于jdk的类加载是双亲委派机制性能较低（加载类时候）
     *		先到java项目的classpath下加载 ，加载不到再到类扩展类加载器中的java_home环境变量的 \lib\ext目录找
     *		到不到再到 启动类加载器 bootStrapClassLoader中 java_home\lib中找
     *		因此自己实现URLClassLoader 是重写了  三个类 ，而不去实现 双亲委派机制
     *		：sharedLoader  用于加载 项目下的 WEB-INF/classes 和  lib 文件
     *		：catalinaLoader
     *		：catalinaLoader
     *		
     *
     * tomcat 定义的 三个类加载器
     * 这三个类加载都继承 jdk的SystemClassLoadeer  ClassLoader是类加载器父类    
     * 
     */
    
    ClassLoader commonLoader = null;
    ClassLoader catalinaLoader = null;
    //sharedLoader 默认是所有web应用的 类加载器 用于加载 web-inf下的classes（项目代码）、lib（依赖jar包）
    ...
    private void initClassLoaders() {
        try {
        	...
        	/*
        	 *  类加载器的 双亲委派机制就是 由createClassLoader("common", null) 方法实现的
        	 *  参数2 ：就是指 上层的classLoader类加载器器，这里为null  意思就是不往上层去找 ，来通过这种方式加载.class
        	 */
   ...
   /**
    ...
    public static void main(String args[]) {
    	//daemon 守护线程 ，当daemon == null 说明tom还未启动
        if (daemon == null) {
            // 实例化
